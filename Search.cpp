...
int Engine::think(int depth,int alpha,int beta,deque<Move>* variation)
{
...
int Engine::AlphaBeta(int depth,int alpha,int beta,Move lastmove,deque<Move>* variation,bool cannull,bool dopv)
{
	//int status = pos.getGameStatus();
	//if(status!=STATUS_NOTOVER)
	//{
    ...
    if(cannull && depth>=3 && pos.underCheck(pos.turn)==false && pieceCount>6) //not endgame
    {
		...
		ply++;
		pos.forceMove(m);
        score = -AlphaBeta(depth-R-1,-beta,-beta+1,m,&dummyline,false,false); //make a null-window search (we don't care by how much it fails high, if it does)
		ply--;
        pos.unmakeMove(m);
		if(score >= beta)
		{
	...
	for(unsigned int i = 0;i<vec.size();i++) //search
	{
		...
		{
			score = -AlphaBeta(depth-1,-alpha-1,-alpha,m,&dummyline,cannull,false);
			if(score>alpha && score<beta) //check for failure
			{
		...
		{
			score = -AlphaBeta(depth-3,-beta,-alpha,m,&dummyline,cannull,dopv);
			//cout << "latemove" << endl;
			if(score>alpha)
			{
